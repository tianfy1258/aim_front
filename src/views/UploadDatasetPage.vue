...
const handleSuccessUpload = (res, file, files) => {
  ...
  setTimeout(() => {
    request({
      url: "afterUploadDataset",
      method: "POST",
      timeout: 20 * 1000,
      data: {
        csvName: filenames[0].slice(-3) === "csv" ? filenames[0] : filenames[1],
        zipName: filenames[0].slice(-3) === "zip" ? filenames[0] : filenames[1],
